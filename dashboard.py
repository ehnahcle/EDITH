...
""")
    ...
    with st.expander("📲 4. HTS별 OCO 예약 주문 방법"):
        st.markdown("""
**키움증권 영웅문**:
1. 매수 체결 후 [주식주문 → OCO주문] 메뉴
2. 종목 선택 → 매도 주문 두 개 동시 등록
   - 1번 주문: 지정가 매도, 익절가 (+15%)
   - 2번 주문: 스탑로스 매도, 손절가 (-3%) 트리거
3. 기간: 1개월 또는 무기한
4. 등록 → 한 쪽 체결되면 다른 쪽 자동 취소

**한국투자증권 eFriend / KIS**:
1. [매매 → OCO 주문] 메뉴
2. 동일 종목에 매도 두 건 (목표가 + 손절가) 입력
3. 유효기간: 무기한 권장
...
**공통**:
- 주문 등록 후 HTS 종료해도 서버에 저장되어 자동 체결됨
- 주문 유효기간을 **5거래일 이상**으로 설정 (시간 청산까지 커버)
""")
